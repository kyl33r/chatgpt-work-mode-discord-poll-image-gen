...
1. Choose a fresh local invocation ID. Put the owner-supplied destination reference, optional stable boundary, and configured qualifying-message limit in a disposable local payload. Do not print, quote, commit, or reproduce those values.
...
3. Read that private request handoff locally. Do not reproduce its destination, boundary, message limit, or any CLI handoff/output in ChatGPT, Discord, logs, documentation, or review comments.
...
6. Submit the bounded batch with `npm run round -- parse-conversation` in `observe` mode, using the same invocation ID. Include only the private observation batch and, on a later rescan, the matching private checkpoint. Continue only on the controlled `wait` or `conversation-complete` result; report only those controlled action/count fields.
...
This skill is independent of Feedback Round collection and must not invoke or modify `get-discord-polls` or `round-start`.
